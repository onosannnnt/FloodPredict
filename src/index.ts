...
app.post("/", (req, res) => {
  ...
  if (water_level <= 5) {
    sendLineMessage(
      ...
      `📢 แจ้งเตือนภัยน้ำท่วมขั้นวิกฤติ! 🚨\nระดับน้ำ: ${water_level} เซนติเมตร\nความชื้น : ${humidity_value[0]} %อุณภูมิ : ${humidity_value[1]} เซลเซียส`
    );
  } else if (water_level < 10) {
    sendLineMessage(
      "Ufd79c6344c9a97376eb756961a7830af",
      `📢 แจ้งเตือนภัยน้ำท่วม: โปรดระมัดระวัง ⚠️🌊\nระดับน้ำ: ${water_level} เซนติเมตร\ความชื้น : ${humidity_value[0]}\nอุณภูมิ : ${humidity_value[1]}`
    );
  }
